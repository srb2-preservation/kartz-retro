...
#ifdef RIDERS
// Write a string using the mario kart font
// NOTE: the text is centered for screens larger than the base width
//
void V_DrawKartString(INT32 x, INT32 y, INT32 option, const char *string, INT32 clrmap)
{
	INT32 w, c, cx = x, cy = y, dupx, dupy, scrwidth = BASEVIDWIDTH;
	const char *ch = string;
	UINT8 lastcolorchar = 0x80; //for dynamic colors
	...
	for (;;)
	{
		...
		c = toupper(c) - KART_FONTSTART;
		if (c < 0 || c >= KART_FONTSIZE)
		{
			cx += 8*dupx; // 'Spacebar' distance.
			continue;
		}
		...
		if (colormap != NULL && ((option & V_TRANSLUCENT) || (option & V_8020TRANS)))
			V_DrawTranslucentMappedPatch(cx, cy, option, kart_font[c], colormap);
		else if (colormap != NULL)
			V_DrawMappedPatch(cx, cy, option, kart_font[c], colormap);
		else if ((option & V_TRANSLUCENT) || (option & V_8020TRANS))
			V_DrawTranslucentPatch(cx, cy, option & ~V_TRANSLUCENT, kart_font[c]);
		else
			V_DrawScaledPatch(cx, cy, option, kart_font[c]);
...
// Find string width from cred_font chars
//
INT32 V_KartStringWidth(const char *string)
{
	INT32 c, w = 0;
	size_t i;
	...
	for (i = 0; i < strlen(string); i++)
	{
		c = toupper(string[i]) - KART_FONTSTART;
		if (c < 0 || c >= KART_FONTSIZE)
			w += 8;
		else
			w += SHORT(kart_font[c]->width);
	}
